			...
			{projects &&
				...
				projects.map((project, index) => {
					...
					return (
						<div
							key={project._id}
							style={{ background: backColor[i][0] }}
							className={'w-full flex flex-col rounded-3xl p-3 text-slate-700'}>
							<section className='grid grid-cols-2 items-center w-full'>
								<p className='justify-self-start capitalize font-medium text-xl'>
									{weekday} {day}
								</p>
								<Button
									Value={BiInfoCircle}
									more='text-3xl p-1'
									bg='justify-self-end'
									styles={{ color: backColor[i][1] }}
									onClick={(e) => Navi(e, project._id)}
								/>
							</section>
							...
							<section className='flex flex-col gap-1 items-center justify-center'>
								...
								<div className='w-full h-2 rounded-lg bg-white'>
									<div
										style={{ width: `${project.progress}%`, background: backColor[i][1] }}
										className={'h-full rounded-lg'}></div>
								</div>
